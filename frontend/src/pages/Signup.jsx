    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < 200) {
            ctx.save();
            ctx.globalAlpha = 0.13 * (1 - dist / 200);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-bl from-slate-900 via-slate-800 to-slate-900 text-white relative overflow-hidden px-4">
     ...
     <div className="absolute inset-0 opacity-20">
      ...
      <div className="relative z-10">
        <div
          className={`relative max-w-md mx-auto bg-slate-900 p-10 border border-slate-700 rounded-2xl shadow-2xl shadow-black transform transition-all duration-700 ${fadeIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}
        >
          {/* Subtle gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-l from-slate-900 via-transparent to-slate-900 rounded-2xl pointer-events-none z-0"></div>
